...
export const useTaskLogic = (tasks, quantites, setQuantites, persistSessionRow, etablissementTerminal) => {
  ...
  const handleQuantityChange = async (taskId, newQ) => {
    try {
      ...
      // La trancheuse est toujours physiquement affectée au Vesuvio, sinon on prend le choix actuel
      ...
      // Gestion automatique du Nettoyage de la trancheuse
  ...
  const handleTimeTracking = async (taskId, demandeurConcret, action) => {
    try {
      ...
      if (action === 'START') {
        ...
        // Envoi blindé dans la table d'historique
        ...
        // Réinitialisation de la ligne suite à la complétion
        const updated = { ...quantites };
